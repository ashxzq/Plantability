...
public class MyPlots_6x8_RemovePlant extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.my_plots_6x8_remove_plant);
        ...
        setPlotValues(); // Sets values of tiles that are stored in the plot object so that they are displayed
        ...
        if (v.getId() == R.id.view_button_layout) {
            ...
            for (int i = 0; i < tileIds.length; i++) {
                ...
                if (v.getId() == tileId) {
                    ...
                    if (curTileData[0].equals("")) {
                        // This means this tile is not currently linked to a plant
                        break; // Nothing to do and we don't want to finish() or start a new activity
                    }
                    // If we are here, this tile is linked to a plant, so lets change that ;)
                    PlotDatabase plotDatabase = PlotDatabase.getPlotDatabaseInstance(getApplicationContext());
                    plotDatabase.plotDao().deletePlot(curPlot); // Remove old version from DB

                    curPlot.putTileData(tileIds[i], "", curTileData[1]);

                    plotDatabase.plotDao().insertPlot(curPlot); // Insert new version to DB

                    setPlotValues(); // Set the plot values again so this change is displayed

                    Intent intent = new Intent(getApplicationContext(), MyPlots_6x8_RemovePlant.class);
                    intent.putExtra("plot_name", plotName);
                    startActivity(intent);
                    finish();
                    break;
